...
// But wait a minute! You said zero-sized, but `sizeof(add) = 1`, why is it a byte large?!
// - zero sized types are very useful, but can be a headache for compiler writers
...
// C-stye function pointers
// - Jump to a pointer
// - Runtime polymorphism
// - unary '+' is for type promotion (e.g bool -> int, lambda type -> function ptr)
int (*add_func_ptr)(int, int) = +add;

int apply_op_ptr(int op(int, int)) {
    return op(2,3);
}
...
int main() {
    ...
    std::cout << "for Virtuals:" << std::endl
              ...
              << "(size is just of the sizeof(void*) = " << sizeof(void*) << " vtable pointer)" << std::endl;
}
