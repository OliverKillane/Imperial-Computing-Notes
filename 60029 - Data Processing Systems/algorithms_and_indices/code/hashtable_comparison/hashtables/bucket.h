...
namespace HashMap {
    ...
    class Buckets {
        ...
        std::vector<BucketNode*> _buckets;
        size_t _size;
    ...
    public:
        ...
        Buckets(size_t initial_buckets = 16) : _buckets(initial_buckets), _size(0) {}
        ...
        friend std::ostream &operator<<(std::ostream &os, const Buckets<K, V, hasher> & ht) {
            os << "Hash Table: " << type<Buckets<K, V, hasher>>() << endl;
        ...
        static_assert(IsHashMap<Buckets, K, V>, "BucketMap is not a hashmap!");
    };
}