...
namespace HashMap {
    ...
    class Probing {
        ...
        void _resize() noexcept {
            ...
            std::vector<std::variant<Empty, Deleted, Entry>> _old_table(_table.size() * 2);
    ...
    public:
        ...
        bool insert(K key, V val) {
            ...
            for (auto probe = Probe::initial(key);;probe = Probe::collision(probe) ) {
                ...
                if (std::holds_alternative<Empty>(slot) || std::holds_alternative<Deleted>(slot)) {
        ...
        friend std::ostream &operator<<(std::ostream &os, const Probing<K, V, Probe> & ht) {
