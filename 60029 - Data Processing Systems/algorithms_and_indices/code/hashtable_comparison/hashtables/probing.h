...
// Probing hashtables / open-addressing.
// - Keys and values stored in n-ary format.
// - Lazy deletion / mark deleted

namespace HashMap {
namespace Probes {
template <typename State> struct ProbeResult {
  size_t hash;
  State state;
};
...
concept IsProbe =
    requires {
      ...
      {
        Probe::collision(
            std::declval<const ProbeResult<typename Probe::State> &>())
        } noexcept -> std::same_as<ProbeResult<typename Probe::State>>;
    };

template <typename K, Hasher<K> hasher, size_t DISTANCE> struct Linear {
  struct State {};
  ...
  static_assert(IsProbe<K, Linear<K, hasher, DISTANCE>>,
                "linear is not a probe");
};

template <typename K, Hasher<K> hasher> struct Quadratic {
  struct State {
  ...
  static ProbeResult<State> initial(const K &key) noexcept {
    ...
    return {.hash = hash, .state = {.original_hash = hash, .collisions = 0}};
  }
  static ProbeResult<State> collision(const ProbeResult<State> &prev) noexcept {
    size_t collisions = prev.state.collisions + 1;
    return {.hash = prev.state.original_hash + collisions * collisions,
            .state = {.original_hash = prev.state.original_hash,
...
template <typename K, Hasher<K> hasher, Hasher<size_t> reHasher> struct ReHash {
  struct State {};
  ...
  static_assert(IsProbe<K, ReHash<K, hasher, reHasher>>,
                "ReHasher is not a probe");
};
}; // namespace Probes

template <typename K, typename V, typename Probe> class Probing {
  ...
  std::vector<std::variant<Empty, Deleted, Entry>> _table;
  size_t _size;

  bool _resize_policy() const noexcept { return _size > _table.size() / 2; }

  void _resize() noexcept {
    // we know the old table has unique entries, and can take advantage of this
    std::vector<std::variant<Empty, Deleted, Entry>> _old_table(_table.size() *
                                                                2);
    std::swap(_old_table, _table);
    for (auto &slot : _old_table) {
      if (holds_alternative<Entry>(slot)) {
        Entry &entry = std::get<Entry>(slot);
        for (auto probe = Probe::initial(entry.key);;
             probe = Probe::collision(probe)) {
...
public:
  ...
  Probing(size_t initial_capacity = 16) : _table(initial_capacity), _size(0) {}
  ...
  bool insert(K key, V val) {
    ...
    for (auto probe = Probe::initial(key);; probe = Probe::collision(probe)) {
      ...
      if (std::holds_alternative<Empty>(slot) ||
          std::holds_alternative<Deleted>(slot)) {
  ...
  V *find(const K &key) noexcept {
    for (auto probe = Probe::initial(key);; probe = Probe::collision(probe)) {
      ...
      if (std::holds_alternative<Empty>(slot)) {
        // end of probe chain,
        return nullptr;
      } else if (std::holds_alternative<Deleted>(slot)) {
        Deleted &deleted = std::get<Deleted>(slot);
        if (deleted.key == key) {
          // key was deleted, if reinserted, it would be here or earlier in the
          // probe chain.
          return nullptr;
        }
      } else if (std::holds_alternative<Entry>(slot)) {
  ...
  friend std::ostream &operator<<(std::ostream &os,
                                  const Probing<K, V, Probe> &ht) {
    os << "Hash Table: " << type<Probing<K, V, Probe>>() << endl;
  ...
  static_assert(IsHashMap<Probing<K, V, Probe>, K, V>, "not a hashmap");
};
} // namespace HashMap