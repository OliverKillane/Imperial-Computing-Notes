...
public:
    Select(PushOperator<Event> *plan, std::function<bool(Event &)> predicate) : plan_(plan), predicate_(predicate) {}
...
public:
    Project(PushOperator<OutputEvent> *plan, std::function<OutputEvent(InputEvent)> function) : plan_(plan), function_(function) {}
...
public:
    WindowTwoStackAggregator(PushOperator<Event> *plan, size_t window_size) : plan_(plan),
...
void example_1()
{
    ...
    Project<int, int> mult(&console, [](auto i)
                           { return i * 3; });
    Select<int> even(&mult, [](auto &i)
                     { return i % 2 == 0; });
...
void example_2()
{
    ...
    Project<int, float> mult(&sum, [](auto i)
